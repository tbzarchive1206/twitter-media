...
const members = ["Group photo", "Sangyeon", "Jacob", "Younghoon", "Hyunjae", "Juyeon", "Kevin", "Q", "Sunwoo", "Eric", "Hwall (2017 - 2019)", "Haknyeon (2017 - 2025)", "New (2017 - 2026)"];
...
function Tile({ item }: { item: Media }) {
  return <article className="media-card">
    {item.kind === "video" ? <iframe className="media-preview media-video" src={previewUrl(item.id)} title={`Video from ${dateLabel(item.date)}`} allow="autoplay; fullscreen" loading="lazy" /> : <a className="media-preview" href={fileUrl(item.id)} target="_blank" rel="noreferrer" aria-label={`Open ${dateLabel(item.date)} in Google Drive`}>
      {item.kind === "image" ? <img src={thumbnail(item.id)} alt="" loading="lazy" /> : <span>{item.kind.toUpperCase()}<br />OPEN ON GOOGLE DRIVE</span>}
    </a>}
    <div className="media-info"><div className="media-name">{dateLabel(item.date)}</div><div className="media-actions"><a href={fileUrl(item.id)} target="_blank" rel="noreferrer">VIEW ↗</a><a href={downloadUrl(item.id)} target="_blank" rel="noreferrer">DOWNLOAD</a></div></div>
  </article>;
}
...
export function TwitterMedia({ data }: { data: Archive }) {
  ...
  const results = useMemo(() => {
    ...
    return data.media.filter((item) => account === "all" || item.account === account)
      ...
      .filter((item) => selectedMembers.every((selected) => item.members.includes(selected)))
  ...
  return <main id="top">
    ...
    <section className="controls" aria-label="Twitter media filters">
      ...
      <div className="member-tabs" aria-label="Filter by one or more members"><button className={selectedMembers.length === 0 ? "selected" : ""} onClick={() => update(() => setSelectedMembers([]))}>ALL</button>{members.map((name) => { const id = memberId(name); return <button className={selectedMembers.includes(id) ? "selected" : ""} onClick={() => toggleMember(id)} key={id} aria-pressed={selectedMembers.includes(id)}>{name}</button>; })}</div>
    </section>
